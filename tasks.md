...
1) колнку comment и date слить вместе и назвать колонку information что олее обстрактное понятие, также в информатион всегда записывать дату первым а это значит что нужно будет добавить валидацию
ты остановился на том что наблюдал такую ситуацию что при изминении количества колонок прийдеться существенно переписать остальной код, а это значит что архитектура кода плохая
и нужно както абстрагироваться от конкретики, потому что сейчас нужно будет переписать фильтры таблиц, остальные таблицы, нужно будет переписать все табличные функции по типу SUM в таблицах куч, и нужно будет переписать сохранение данных
...
21) В таблице на onEdit добавить покраску полей типа плюсов поля краситься в зелений минуслвие в красний, (поля те что в операциях ) и также сохраняються с покраслм
...
23) нужно записывать дату привельно так  mm-dd-yyyy\04-26-2019 или так yyyy-mm-dd\2019/04/26
